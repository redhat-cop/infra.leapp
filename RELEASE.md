...
- Create the label **`release-on-merge`** under **Issues → Labels** if it does not exist (the changelog PR is labeled automatically).
...
4. The workflow will:
   ...
   - open a **pull request** against the default branch, labeled **`release-on-merge`**.

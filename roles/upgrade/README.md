...
| post_upgrade_python_interpreter | /usr/libexec/platform-python | For RHEL 7 to 8 upgrades and above, /usr/bin/python is discovered but not available post upgrade, so ansible_python_interpreter is set to this value post upgrade reboot prior to reconnecting. |

...
Note that the `upgrade` role by default requires the leapp report generated by the `analysis` role to have no inhibitors. You can let the `upgrade` role ignore the `analysis` role results and recheck for possible inhibitors by setting the `leapp_check_analysis_results` variable to false.
...
| leapp_pre_upgrade_update                      | Boolean | true                           | Whether to make the role clear the versionlock list, update all packages and optionally reboot if any packages were updated. This is required for a successful upgrade. |

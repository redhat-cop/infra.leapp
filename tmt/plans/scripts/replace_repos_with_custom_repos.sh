...
if [ -z "${!BASEOS_VAR}" ] && [ -z "${!APPSTREAM_VAR}" ]; then
    BASEOS_VAR="RHEL_${OS_MAJOR_VERSION}_LATEST_BASEOS_REPO_URL"
    APPSTREAM_VAR="RHEL_${OS_MAJOR_VERSION}_LATEST_APPSTREAM_REPO_URL"
elif [ -z "${!BASEOS_VAR}" ] || [ -z "${!APPSTREAM_VAR}" ]; then
    echo "One of the repositories is not set:"
    echo "BASEOS_VAR($BASEOS_VAR) is $([[ -n ${!BASEOS_VAR} ]] && echo set || echo unset)"
    echo "APPSTREAM_VAR($APPSTREAM_VAR) is $([[ -n ${!APPSTREAM_VAR} ]] && echo set || echo unset)"
    exit 1
fi
{

...
#   Default is "-vv" - adjust in plans/main.fmf environment section
...
rlJournalStart
    rlPhaseStartSetup "Setup control node"
        ...
        rlWaitForCmd "ansible-galaxy collection install -r $coll_path/meta/collection-requirements.yml -vv" -m 5 -t 600

        lsrSetAnsibleInjectFactVars "$SR_ANSIBLE_INJECT_FACT_VARS"
        lsrPrepareNodesInventories
        managed_nodes=$(lsrGetManagedNodes)

        # Write shared state for downstream tests
        {
            echo "export coll_path=$coll_path"
            echo "export managed_nodes=\"$managed_nodes\""
            echo "export SR_SKIP_TAGS=\"$SR_SKIP_TAGS\""
            echo "export SR_ANSIBLE_VERBOSITY=\"$SR_ANSIBLE_VERBOSITY\""
            echo "export SR_ANSIBLE_VER=\"$SR_ANSIBLE_VER\""
            echo "export TMT_PLAN=\"$TMT_PLAN\""
            echo "export SR_TFT_DEBUG=\"$SR_TFT_DEBUG\""
            echo "export SR_REPORT_ERRORS_URL=\"$SR_REPORT_ERRORS_URL\""
            echo "export SR_RESERVE_SYSTEMS=\"$SR_RESERVE_SYSTEMS\""
        } > "$TMT_PLAN_DATA/leapp_test_env.sh"
        rlLog "Wrote shared state to $TMT_PLAN_DATA/leapp_test_env.sh"
    rlPhaseEnd
rlJournalEnd
